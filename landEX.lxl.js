...
// 读取已有的配置文件
if (File.exists('./plugins/landEX/config.json')) {
  configAPI.reload();
} else {
  configAPI.save();
}
...
const i18n = {
  ...
  reload() {
    if (File.exists('./plugins/landEX/i18n/' + configAPI.data.common.language + '.json')) {
      i18n.data = data.parseJson(File.readFrom('./plugins/landEX/i18n/' + configAPI.data.common.language + '.json'));
    }
  },
};
if (File.exists('./plugins/landEX/i18n/' + configAPI.data.common.language + '.json')) {
 ...
 * @type {boolean}
 */
let enableOrg = configAPI.data.common.useOrgnization && ll.require('orgEX.lxl.js');
/**
 * @type {boolean}
 */
...
if (enableOrg)
  var orgAPI = {
    getOrgNum: ll.import('orgEX','orgEX_getPlayerOrgNum'),
    isOwner: ll.import('orgEX','orgEX_playerIsOwner'),
    orgGetMoney: ll.import('orgEX','orgEX_orgGetMoney'),
    orgAddMoney: ll.import('orgEX','orgEX_orgAddMoney'),
    getOrgName: ll.import('orgEX','orgEX_getOrgName'),
  };
...
if (File.exists('./plugins/landEX/owner.json')) {
  belongToApi.reload();
} else {
  belongToApi.save();
}
...
if (File.exists('./plugins/landEX/priviteLandData.json')) {
...
if (File.exists('./plugins/landEX/orgLandData.json')) {
...
const ChunkInterface = {
  ...
  load() {
    logger.log('加载缓存的索引表...');
    this.data = JSON.parse(File.readFrom('./plugins/landEX/landIndex.json'));
  },
  reload() {
    logger.log('正在构建索引表...');
    // 设置计数
  ...
  getChunksLand(x, z, dim, org) {
    ...
    if (
      ...
      !this.data[org ? 'organization' : 'private'][dim]['C' + String(chunkX)] ||
      !this.data[org ? 'organization' : 'private'][dim]['C' + String(chunkX)]['C' + String(chunkZ)]
    ) {
...
} else if (File.exists('./plugins/landEX/landIndex.json')) {
  ChunkInterface.load();
} else {
  ChunkInterface.reload();
  ChunkInterface.save();
...
function drawCube(posMin, posMax, dimid, color, is2D) {
  if (is2D) {
    ...
    ps.drawAxialLine(mc.newIntPos(posMin.x, 128, posMin.z, dimid), Direction.POS_Y, 192, color);
    //东北角侧
    ps.drawAxialLine(mc.newIntPos(posMax.x + 1, 128, posMin.z, dimid), Direction.NEG_Y, 192, color);
    ps.drawAxialLine(mc.newIntPos(posMax.x + 1, 128, posMin.z, dimid), Direction.POS_Y, 192, color);
    //西南角侧
    ps.drawAxialLine(mc.newIntPos(posMin.x, 128, posMax.z + 1, dimid), Direction.NEG_Y, 192, color);
    ps.drawAxialLine(mc.newIntPos(posMin.x, 128, posMax.z + 1, dimid), Direction.POS_Y, 192, color);
    //东南角侧
    ps.drawAxialLine(mc.newIntPos(posMax.x + 1, 128, posMax.z + 1, dimid), Direction.NEG_Y, 192, color);
    ps.drawAxialLine(mc.newIntPos(posMax.x + 1, 128, posMax.z + 1, dimid), Direction.POS_Y, 192, color);
  } else {
    ps.drawCuboid(mc.newIntPos(posMin.x, posMin.y, posMin.z, dimid), mc.newIntPos(posMax.x, posMax.y, posMax.z, dimid), color);
  }
}
// 地皮进入离开显示
      ...
      case 'reEnclosing':
        {
          ...
          let landData = playerState[player.xuid].editingLand.isOrg
            ? OlandDataInterface.data[playerState[player.xuid].editingLand.landId]
            : pLandDataInterface.data[playerState[player.xuid].editingLand.landId];
      ...
      if (!result) {
        ...
        if (result === playerState[player.xuid].inOLand) {
          ...
          if (
            ...
            orgAPI.getOrgNum(player.xuid) === landData.settings.owner &&
  ...
  switch (action) {
    case 'setpos1':
      ...
      if (pos.dimid !== playerState[player.xuid].enclosure.dim) {
        ...
        // 维度相同，仅写入坐标
        playerState[player.xuid].enclosure.posA = [pos.x, pos.y, pos.z];
        if (playerState[player.xuid].enclosure.posA.length !== 0 && playerState[player.xuid].enclosure.posB.length !== 0 && configAPI.data.common.useDrawLine) {
          // 画线
      ...
      player.sendForm(fm, (pl, dt) => {
        ...
        if (dt == null) {
          ...
          if (
            ...
            configAPI.data.common.useDrawLine
          ) {
            // 画线
            let posInter = twoPosFormat(playerState[player.xuid].enclosure.posA, playerState[player.xuid].enclosure.posB);
            drawCube(
    ...
    case 'confirm':
      if (playerState[player.xuid].state === 'enclosing' || playerState[player.xuid].state === 'reEnclosing') {
        ...
        if (configAPI.data.common.allow2D && configAPI.data.common.allow3D) {
          ...
          playerState[player.xuid].state === 'enclosing' ? SelectEncloseType(player) : ReEnclosingScan(player);
...
function SelectEncloseType(player) {
  ...
  if (belongToApi.getLand(player.xuid).length >= configAPI.data.limit.maxLands) {
    let fm = mc
      .newSimpleForm()
      .setTitle(i18n.$t('enclose.confirm.countover.title'))
      .setContent(i18n.$t('enclose.confirm.countover.info', [configAPI.data.limit.maxLands]));
    player.sendForm(fm, () => {});
  }
  // 大小限制
  if (playerState[player.xuid].enclosure.type2D) {
...
function enclosePayment(player, posInterface, orgNum = null) {
  ...
  if (!orgNum) {
    ...
    if (moneyUni.get(player) < price && orgAPI.orgGetMoney(orgNum) < price) {
      ...
      player.sendModalForm(
        i18n.$t('enclose.payment.title'),
        i18n.$t('enclose.payment.ori', [price, orgAPI.orgGetMoney(orgNum)]),
        i18n.$t('common.confirm'),
        i18n.$t('common.cancel'),
        (pl, dt) => {
          if (dt == null) {
      ...
      player.sendModalForm(
        i18n.$t('enclose.payment.title'),
        i18n.$t('enclose.payment.choose', [price, moneyUni.get(player), orgAPI.orgGetMoney(orgNum)]),
...
function ManageLand(player, landId, isOrg = false) {
  ...
  if (configAPI.data.common.allowLandTeleport) fm.addButton(i18n.$t('manage.main.teleport'), 'textures/items/ender_eye');
...
function LandInfo(player, landId, isOrg = false) {
  ...
  fm.setContent(
    i18n.$t('manage.info.desp', [
      ...
      landData.range.dimid,
      isOrg ? i18n.$t('this.manage.choose.org') : i18n.$t('this.manage.choose.pri'),
      isOrg ? orgAPI.getOrgName(landData.settings.owner) || i18n.$t('manage.info.noOrg') : data.xuid2name(landData.settings.owner),
      landData.settings.describe || i18n.$t('manage.info.noDesp'),
      landData.range.type2D ? i18n.$t('common.YES') : i18n.$t('common.NO'),
      landData.teleport.length === 0 ? i18n.$t('manage.info.noDesp') : String(landData.teleport),
    ])
  );
  player.sendForm(fm, (pl, dt) => {
    if (dt == null) {
  ...
  player.sendForm(fm, (pl, dt) => {
    ...
    if (dt === 0) {
      if (landData.teleport.length !== 0) {
        ...
        let pos = player.blockPos;
        if (
          (isOrg && !OlandDataInterface.isPosInLand(pos.x, pos.y, pos.z, pos.dimid, landId)) ||
          (!isOrg && !pLandDataInterface.isPosInLand(pos.x, pos.y, pos.z, pos.dimid, landId))
        ) {
          //不在地里
          let fm = mc.newSimpleForm().setTitle(i18n.$t('manage.teleport.title')).setContent(i18n.$t('manage.teleport.clearSuccess'));
          player.sendForm(fm, (pl, dt) => {
...
function ExplodeHander(landData, pos, range) {
  ...
  if (landData.range.type2D && (distance.nx > range || distance.px > range || distance.nz > range || distance.pz > range)) {
    ...
    !landData.range.type2D &&
    (distance.nx > range || distance.px > range || distance.nz > range || distance.pz > range || distance.ny > range || distance.py > range)
  ) {
    //范围之外
    return true;
  }
...
function recycleLand(player, landId, isOrg) {
  ...
  player.sendModalForm(
    ...
    i18n.$t('recycle.gui.desp', [landId, isOrg ? i18n.$t('this.manage.choose.org') : i18n.$t('this.manage.choose.pri'), landData.settings.name, price]),
    i18n.$t('common.YES'),
    i18n.$t('common.NO'),
    (pl, dt) => {
      if (!dt) {
...
function ReEnclosingScan(player) {
  ...
  let pWhiteList = playerState[player.xuid].editingLand.isOrg
    ? getPLandinRange(
        twoPosFormat(landData.range.min_position, landData.range.max_position),
        playerState[player.xuid].enclosure.dim,
  ...
  if (!playerState[player.xuid].editingLand.isOrg) {
    ...
    if (moneyUni.get(player) < price) {
      ...
      player.sendModalForm(
        i18n.$t('enclose.payment.title'),
        i18n.$t('enclose.payment.pri', [String(price), moneyUni.get(player)]),
        i18n.$t('common.confirm'),
        i18n.$t('common.cancel'),
        (pl, dt) => {
          if (dt == null) {
    ...
    if (moneyUni.get(player) < price && orgAPI.orgGetMoney(landData.settings.owner) < price) {
      ...
      player.sendModalForm(
        i18n.$t('enclose.payment.title'),
        i18n.$t('enclose.payment.ori', [String(price), orgAPI.orgGetMoney(landData.settings.owner)]),
        i18n.$t('common.confirm'),
        i18n.$t('common.cancel'),
        (pl, dt) => {
          if (dt == null) {
      ...
      player.sendModalForm(
        i18n.$t('enclose.payment.title'),
        i18n.$t('enclose.payment.choose', [String(price), moneyUni.get(player), orgAPI.orgGetMoney(landData.settings.owner)]),
        i18n.$t('enclose.pay.self'),
        i18n.$t('enclose.pay.org'),
        (pl, dt) => {
          if (dt == null) {
...
const playerDB = {
  ...
  data: {},
  save() {
    File.writeTo('./plugins/landEX/playerDB.json', data.toJson(this.data));
  },
  reload() {
    this.data = data.parseJson(File.readFrom('./plugins/landEX/playerDB.json'));
  },
};
// 读取已有的配置文件
if (File.exists('./plugins/landEX/playerDB.json')) {
  configAPI.reload();
} else {
  configAPI.save();
}
  ...
  player.sendForm(fm, (pl, dt) => {
    ...
    if (dt === 1) {
      ...
      pl.sendForm(fm, (plar, dt) => {
        ...
        pLandDataInterface.save();
        belongToApi.save();
        let fm = mc.newSimpleForm().setTitle(i18n.$t('shared.title')).setContent(i18n.$t('shared.deleteSuccess'));
      ...
      playerSelector(pl, (pla, xuid) => {
        if (landData.share.includes(xuid)) {
          let fm = mc.newSimpleForm().setTitle(i18n.$t('shared.title')).setContent(i18n.$t('shared.hasInclude'));
          ...
          let fm = mc.newSimpleForm().setTitle(i18n.$t('shared.title')).setContent(i18n.$t('shared.success'));
          pla.sendForm(fm, (plar, dt) => {
            sharedManageHander(plar, landId, isOrg);
          });
        }
...
function landResellManager(player, landId, isOrg) {
  ...
  player.sendForm(fm, (pl, dt) => {
    ...
    if (dt[1] === '') {
      ...
      pLandDataInterface.save();
      let fm = mc.newSimpleForm().setTitle(i18n.$t('resell.title')).setContent(i18n.$t('resell.success'));
      player.sendForm(fm, (pla, dt) => {
        ManageLand(pla, landId, isOrg);
      });
      return;
    ...
    if (isNaN(dt[1])) {
      //不是数字
      let fm = mc.newSimpleForm().setTitle(i18n.$t('resell.title')).setContent(i18n.$t('resell.buy.isNaN'));
      player.sendForm(fm, (pla, dt) => {
...
function UUIDManage(player) {
  ...
  player.sendForm(fm, (pl, dt) => {
    ...
    if ((isOrg && !OlandDataInterface.data[LandId]) || (!isOrg && !pLandDataInterface.data[LandId])) {
      let fm = mc.newSimpleForm().setTitle(i18n.$t('dashboard.main.title')).setContent(i18n.$t('dashboard.uuid.nofound'));
      player.sendForm(fm, (pla, dt) => {
        UUIDManage(pla);
      });
      return;
